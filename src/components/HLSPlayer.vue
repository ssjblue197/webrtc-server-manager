    ...
    <div class="mb-4 ml-2 mr-2 playerWrap" :style="isRebooting ? 'background-color: white; opacity: 0.3;' : ''">
        ...
        <b-row style="display: block; margin-bottom: -10px; background-color: #B3B8BC;"> 
          <video 
            ...
            :id="`hls-video${this.streamID + 'video' + this.channelSelected.name}`" autoplay muted playsinline width="100%" @dblclick="viewStream()"></video>
          <div
            v-show="showIconReload"
...
export default {
  ...
  methods: {
    ...
    viewStream() {
      this.$emit('showPlayer', { selectChannel: this.channelSelected, type: 'hls' });
    },
